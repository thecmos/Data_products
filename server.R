...
shinyServer(function(input, output) {
  ...
  output$plot1 <- renderPlot({
    ...
    lab_sz<- ifelse(input$show_labels,5.25,0)
    
    g<-ggplot(caps,aes(x=lng,y=lat,color=city))+geom_point(aes(size=population,alpha=1/3))+theme(legend.position="none") +geom_text(aes(label=city
    ),hjust=0.5, vjust=-1,size=lab_sz)+ scale_x_continuous(limits = c(-180, 180))+scale_y_continuous(limits = c(-90, 90))+ labs(x=xlab,y=ylab,title=main,subtitle="Source: https://simplemaps.com/data/world-cities")+ guides(color=guide_legend("Country"))
